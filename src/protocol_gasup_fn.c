...
static UINT32 ptl_gasup_pack_meterdata_nak(const PTL_GASUP_MSG *msg,
	...
	case PTL_GASUP_FN_GET_ONE_RD_DATA:
		...
		ptr += 5;
		break;
	case PTL_GASUP_FN_GET_MONTHDATA: /// month data
		if (msg->datalen < 17)
			return 0;
		...
		INT32 max_outlen, INT32 *datalen, INT32 max_datalen) /// int8-> unsigned char? /// 0x68 96+8 = 104<255
{
	...
	GASMETER_CJT188_901F di_data;
	BYTE addbuf[MAX_HEXDATA_BUF]; /// 64

	if (msg == NULL || msg->data == NULL || msg->datalen < 17 || outdata == NULL /// datalen 17
	|| max_outlen < 24 || max_datalen < 1)
		return 0;
	if (memcmp(&msg->address[2], msg->data, 5) == 0) { /// step to step to judege the message /// if the data concentrator id is the same to the substation id
		...
		for (i = 0; i < PTL_GASUP_MAX_PACK_CNT; i++) {
			for (blockidx = next_blockidx; blockidx < MAX_GASMETER_MON_CNT;
					blockidx++) { /// 120 months
				if (fmon_get_data(blockidx, mtidx, 0x901F, NULL)) /// if has month data
					break;
			} /// if has month data break
			if (blockidx >= MAX_GASMETER_MON_CNT) /// if block index is not valid
				break;
			pstart = ptr;
			bcd_be_stoc(ptr, 0); /// response code 0000(success)
			ptr += 2;
			memcpy(ptr, msg->data, 17);
			ptr += 17;
			lastframe_ptr = ptr;
		...
		if (frame_cnt > 0) {
			...
			return frame_cnt; /// sending frame counts
		} else {
			datalen[0] = ptl_gasup_pack_meterdata_nak(msg, ptr, max_outlen);
			if (datalen[0] > 0)
				return 1;
...
/*
 const PTL_GASUP_MSG *msg, 	消息指针
 INT8 *outdata, 			响应信息
 INT32 max_outlen, 			响应长度
 INT32 *datalen, 			报文长度域
 INT32 max_datalen			
 */
